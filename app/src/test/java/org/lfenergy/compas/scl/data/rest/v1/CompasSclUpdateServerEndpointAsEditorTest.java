...
import org.lfenergy.compas.scl.data.rest.v1.websocket.UpdateRequestEncoder;
import org.lfenergy.compas.scl.data.rest.v1.websocket.UpdateResponseDecoder;
...
class CompasSclUpdateServerEndpointAsEditorTest {
    ...
    void updateSCL_WhenCalled_ThenExpectedResponseIsRetrieved() throws Exception {
        ...
        when(service.update(sclFileTye, UUID.fromString(ID_VALUE), cst, CompasSclDataResourceAsEditorTest.USERNAME, comment, sclData))
                .thenReturn(sclData);
        ...
        try (Session session = ContainerProvider.getWebSocketContainer().connectToServer(Client.class, uri)) {
            ...
            verify(service, times(1)).update(sclFileTye, UUID.fromString(ID_VALUE), cst, CompasSclDataResourceAsEditorTest.USERNAME, comment, sclData);
        }
    }
